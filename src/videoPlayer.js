 ...
 *  @version v1.2.0
 *  @date 2023-12-6
 *  @desc 源码配套插件, 该版本主要涉及兼容插件播放器的登录功能。
 */
(function () {
    window.dhPlayerControl = Object.assign({
        ...
        DHPlayerVersion: '',
        pkgDHPlayerVerion: [2311300922, 2311300922], // 配套的插件版本号
        pluginLoginInfo: {},
        isPIframe: false,
    ...
    function getVersionInfo() {
        ...
        if (window.dhPlayerControl.pkgDHPlayerVerion[0] > window.dhPlayerControl.DHPlayerVersion) {
            return {
                isEqual: false,
                code: -1,
                message: '当前电脑上的插件版本过低, 请卸载插件并安装对应版本的插件!'
            }
        }
        if (window.dhPlayerControl.pkgDHPlayerVerion[window.dhPlayerControl.pkgDHPlayerVerion.length - 1] < window.dhPlayerControl.DHPlayerVersion) {
            return {
                isEqual: false,
                code: 1,
                message: '当前电脑上的插件版本过高, 如果出现问题, 请卸载安装对应版本的插件!'
            }
        }
    }
     ...
     * @desc 只有NVS{3}，NVR{6}，smartNVR{14}[已弃用]，IVSS{43}能对通道进行对讲。
     */
     ...
     * 能否云台控制
     */
    function canPanTilt(cameraType, capability) {
        // 球机
        if (String(cameraType) === '2') {
            return true
        }
    ...
    function socketOpen() {
        ...
        //     console.log("----------无法与插件建立连接-------");
        //     window.dhPlayerControl.windowState = 'wsError'
        //     for (var key in window.dhPlayerControl.videoList) {
        //         var currentThis = window.dhPlayerControl.videoList[key]
        //         currentThis.setting.createError && currentThis.setting.createError({
        //             code: 1001,
    ...
    function socketMessage(evt) {
        ...
        if (data.info && typeof data.info.hwnd === 'number') {
            ...
            switch (data.method) {
                case 'window.getLoginState':
                    if(data.info.data.state) {
                        hwndInfo.createSuccess && hwndInfo.createSuccess(getVersionInfo())
                        return
                    }
                    videoInfo.loginServer();
                case 'window.loginServer.notify':
                    if (data.info.success) {
                        hwndInfo.createSuccess && hwndInfo.createSuccess(getVersionInfo())
                    } else {
                        window.dhPlayerControl.pluginLoginInfo = {}
                        hwndInfo.createError && hwndInfo.createError({
                            code: 1004,
                            data: null,
                            message: '播放器登录失败，请检查登录信息是否正确',
                            success: false
                        })
                    }
                    break;
                ...
                case 'video.change.substream':
                    if (channelInfo.byUrl) {
                        return hwndInfo.dhPlayerMessage && hwndInfo.dhPlayerMessage(channelInfo, {...window.dhPlayerControl.dhMessage[706], streamType: substream})
                    }
    ...
    var VideoPlayer = function (option) {
        ...
        if (this instanceof VideoPlayer) {
            var _setting = {
                ...
                isDestroyed: false, // 当前是否有正在销毁的插件
    ...
    VideoPlayer.fn = VideoPlayer.prototype = {
        ...
        _update(param) {
            ...
            let { windowType, isResetConnect, request, division, visible, draggable, showBar, shieldClass, coverShieldClass, parentIframeShieldClass } = param
            // 断线重连 (不对外开放，默认支持断线重连)
                    ...
                    },
                    {
                        onSuccess: function (data) {
                            if (data.data && typeof data.data.hwnd === 'number') {
                                ...
                                console.log(`==================创建成功 hwnd: ${hwnd}==================`);
                                console.log(`插件版本: ${window.dhPlayerControl.DHPlayerVersion}`);
                                that.setting.hwnd = hwnd
                                that.setting.isDestroyed = false
                            ...
                            if (that.setting.usePluginLogin) {
                                // 登录状态
                                let {host: nhost, port: nport, username: nuser} = that.setting.pluginLoginInfo
                                let {host: ohost, port: oport, username: ouser} = window.dhPlayerControl.pluginLoginInfo
                                // 如果登录信息一样
                                if(nhost === ohost && nport === oport && nuser === ouser) {
                                    that.getLoginState()
                                } else {
                                    window.dhPlayerControl.pluginLoginInfo = {
                                        host: nhost,
            ...
            let sendVideo = () => {
                this.send({
                    ...
                    info: {
                        ...
                        count,
                        deviceCode: option.channelId.split('$1$0$')[0],
                        deviceType: option.deviceType,
                        talkType: getTalkType(option.deviceType),
                        canPanTilt: canPanTilt(option.cameraType, option.capability)
                    }
                })
            }
            // 非集成情况
        ...
        loginServer: function () {
            this.logoutServer().then(res => {
                this.send({
                    method: 'window.loginServer',
                    info: {
                        host: this.setting.pluginLoginInfo.host,
                        port: this.setting.pluginLoginInfo.port,
